...
import { ArrowLeft, MapPin, Save, CheckCircle2, ChevronDown, Calendar, Hash, Layers, ChevronRight, Clock } from "lucide-react";
...
export default function VisitRegistrationPage({ params }: { params: { id: string } }) {
  ...
  // Fetch current lead data to check for existing visit
  useEffect(() => {
    fetch(`/api/leads/${params.id}`)
      ...
      .then(data => {
        if (data.visitDate) {
          ...
          // Set Project
          const project = PROJECTS.find(p => p.name === data.visitProject);
          ...
          setVisitTime(d.toTimeString().split(' ')[0].substring(0, 5));
        }
      })
      .catch(err => console.error("Error fetching lead for rescheduling:", err));
  }, [params.id]);
    ...
    <div className="font-display bg-[#f6f8f8] text-slate-900 min-h-screen relative">
      <div className="relative flex h-auto min-h-screen w-full flex-col overflow-x-hidden max-w-lg mx-auto bg-white shadow-xl">
        <header className="flex items-center p-4 border-b border-primary/10 justify-between sticky top-0 bg-white/80 backdrop-blur-md z-10">
          ...
          <h2 className="text-slate-900 text-lg font-black leading-tight tracking-tight flex-1 text-center uppercase">
            {isRescheduling ? "Reagendar Visita" : "Programar Visita"}
          </h2>
